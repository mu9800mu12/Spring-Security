...
public class LoginController {
    ...
    public ResponseEntity<CommonResponse> loginSuccess(@AuthenticationPrincipal AuthInfo authInfo,
        ...
        log.info("[유저 컨트롤러 ] : loginSuccess 시작!");

        // 스프링 시큐리티에 저장된 정보 가져오기
        UserInfoDTO rDTO = Optional.ofNullable(authInfo.getUserInfoDTO())
    ...
    public ResponseEntity<CommonResponse> loginInfo(HttpSession session) {
        ...
        // 세션 값 전달할 데이터 구조 만들기
        UserInfoDTO dto = UserInfoDTO.builder().userId(userId).userName(userName).roles(roles)
                .build();
